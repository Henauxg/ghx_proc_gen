...
pub fn spawn_debug_grids<T: DirectionSet + Clone + Sync + Send + 'static>(
    ...
    debug_grids: Query<(Entity, &Grid<T>, &DebugGridView), Added<DebugGridView>>,
) {
    for (grid_entity, grid, grid_view) in debug_grids.iter() {
        let mut lines = vec![];
        for y in 0..=grid.def.size_y() {
            ...
            let mut to = Vec3::new(
                0.,
                y as f32,
                -(grid.def.size_z() as f32) * grid_view.node_size.z,
            );
            for x in 0..=grid.def.size_x() {
                from.x = grid_view.node_size.x * x as f32;
                to.x = from.x;
                lines.push((from, to));
            }
            from = Vec3::new(0., y as f32, 0.);
            to = Vec3::new(
                grid.def.size_x() as f32 * grid_view.node_size.x,
                y as f32,
                0.,
            );
            for z in 0..=grid.def.size_z() {
                from.z = -(grid_view.node_size.z * z as f32);
                to.z = from.z;
                lines.push((from, to));
            }
        }
        for x in 0..=grid.def.size_x() {
            ...
            let mut to = Vec3::new(
                x as f32,
                grid.def.size_y() as f32 * grid_view.node_size.y,
                0.,
            );
            for z in 0..=grid.def.size_z() {
                from.z = -(grid_view.node_size.z * z as f32);
